...
x_first_part = [[i, k] for i, k in zip(first_value,  x_time)]
...
x_test_first_part = [[i, k] for i, k in zip(test_first_value,  x_test_time)]

x_enc_first_part_test = enc.transform(x_test_first_part)
df_x_first_part_test = pd.DataFrame(x_test_first_part, index=x_test_second.index)
